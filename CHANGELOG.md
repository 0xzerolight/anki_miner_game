...
- **The speaker-tag filter (Game profile -> Remove a leading 【name】 speaker tag) now also strips a `name:`/`name：` prefix before an opening quote.** The Agent hooker's STEINS;GATE script sends dialogue as `倫太郎: 「…」` rather than LunaTranslator's `【倫太郎】「…」`; only the bracketed form was removed before, so the speaker name leaked into every cue and card. Lines with an unrelated colon (`注意：これは…`, `A: B`) are unaffected.
