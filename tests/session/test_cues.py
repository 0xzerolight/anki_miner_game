...
def test_spec_table_row(shown_ms, length_ms, gap_ms):
    ...
    cues = build_cues(lines_at(first, second), stop_ms=second + 60_000, shift_ms=HOOK, cfg=DEFAULTS)
...
def test_skip_threshold_is_exclusive(shown_ms, kept):
    cues = build_cues(lines_at(10_000, 10_000 + shown_ms), stop_ms=90_000, shift_ms=HOOK, cfg=DEFAULTS)
...
def test_cues_are_numbered_from_one_and_carry_text_and_source():
    ...
    cues = build_cues(lines, stop_ms=20_000, shift_ms=HOOK, cfg=DEFAULTS)
    assert cues == [
...
def test_no_lines_gives_no_cues():
    assert build_cues([], stop_ms=60_000, shift_ms=HOOK, cfg=DEFAULTS) == []
...
def test_burst_of_click_through_lines_is_dropped_as_a_whole():
    ...
    cues = build_cues(lines, stop_ms=40_000, shift_ms=HOOK, cfg=DEFAULTS)
    assert spans(cues) == [
...
def test_a_dropped_line_never_shortens_its_neighbour():
    ...
    cues = build_cues(lines, stop_ms=30_000, shift_ms=HOOK, cfg=DEFAULTS)
...
def test_last_line_is_measured_against_the_stop_offset(stop_ms, expected):
    assert spans(build_cues(lines_at(10_000), stop_ms=stop_ms, shift_ms=HOOK, cfg=DEFAULTS)) == expected
...
def test_last_kept_line_ends_against_the_stop_when_later_lines_are_dropped():
    ...
    cues = build_cues(lines_at(10_000, 12_000), stop_ms=12_200, shift_ms=HOOK, cfg=DEFAULTS)
...
def test_lines_past_the_stop_are_dropped_and_no_cue_ends_past_it():
    ...
    cues = build_cues(lines_at(1_000, 5_000), stop_ms=900, shift_ms=HOOK, cfg=DEFAULTS)
    assert cues == []
    cues = build_cues(lines_at(1_000, 5_000), stop_ms=3_000, shift_ms=HOOK, cfg=DEFAULTS)
...
def test_start_never_moves_before_the_previous_start():
    # Offsets are clamped non-decreasing by the record clock; the cue builder does not rely on it.
    cues = build_cues(lines_at(5_000, 4_000, 9_000), stop_ms=30_000, shift_ms=HOOK, cfg=DEFAULTS)
    assert spans(cues) == [("line 1", 5_000, 8_650), ("line 2", 9_000, 24_000)]


def test_settings_change_cap_and_gap():
    cfg = CueSettings(max_cue_seconds=5, end_gap_ms=0)
    cues = build_cues(lines_at(1_000, 3_000, 30_000), stop_ms=60_000, shift_ms=HOOK, cfg=cfg)
...
def test_minimum_cue_length_applies_when_the_next_start_allows():
    ...
    cues = build_cues(lines_at(0, 700), stop_ms=60_000, shift_ms=HOOK, cfg=DEFAULTS)
