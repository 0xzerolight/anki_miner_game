...
| `switch_streaming.jsonl` | Switch with a stream active | As `switch_recording.jsonl` with `SetStreamServiceSettings` (`rtmp_custom` to a local `ffmpeg -listen 1` sink, `tools/m0/obs_scenarios.py rtmp-sink`) and `StartStream` instead of recording. A colour source was added to `Untitled`'s scene first (direct client), since OBS asks "no sources?" in a modal before streaming from an empty scene |
| `switch_replay_buffer.jsonl` | Switch with the replay buffer active | As `switch_streaming.jsonl` with `RecRB=true` seeded in `Untitled`'s `basic.ini` before launch and `StartReplayBuffer` |
