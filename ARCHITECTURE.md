...
`session/cues.py` `build_cues()` starts each cue when its line arrived and ends it shortly before the next kept line (`end_gap_ms`), capped at `max_cue_seconds`; lines shown for under 300 ms (skip mode) are dropped. `session/srt_writer.py` writes UTF-8 without BOM, via a temporary file and `os.replace`.
